    ...
    */
    ...
    public function buyStock($data, $user)
    {
        ...
        try {
            ...
            //pivot to alt shop if necessary
            if ($shop->shop_type && method_exists($shop->service, 'handleAltStock')) {
                $service = $shop->service;
                if (! $service->handleAltStock($shop, $data, $user, $shopStock)) {
                    foreach ($service->errors()->getMessages()['error'] as $error) {
                        flash($error)->error();
                    }
                    throw new \Exception("Failed to handle shop stock.");
                }
            ...
            //the rest applies to all other shops so just go back to usual.
