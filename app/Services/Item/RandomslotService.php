...
class RandomslotService extends Service
{
    ...
    public function getTagData($tag)
    {
        ...
        if (isset($tag->data['is_sellable']) && $tag->data['is_sellable'] == "true") {$characterData['is_sellable'] = 1;} else {
            $characterData['is_sellable'] = 0;
        }

        if (isset($tag->data['is_tradeable']) && $tag->data['is_tradeable'] == "true") {$characterData['is_tradeable'] = 1;} else {
            $characterData['is_tradeable'] = 0;
        }

        if (isset($tag->data['is_giftable']) && $tag->data['is_giftable'] == "true") {$characterData['is_giftable'] = 1;} else {
            $characterData['is_giftable'] = 0;
        }

        if (isset($tag->data['is_visible']) && $tag->data['is_visible'] == "true") {$characterData['is_visible'] = 1;} else {
            ...
            foreach ($stacks as $key => $stack) {
                ...
                if ((new InventoryManager)->debitStack($stack->user, 'Slot Used', ['data' => ''], $stack, $data['quantities'][$key])) {
                    ...
                    for ($q = 0; $q < $data['quantities'][$key]; $q++) {
                        ...
                        $characterData = $stack->item->tag('randomslot')->data;
                            ...
                            foreach ($characterData['groups'] as $group) {
                                ...
                                for ($i = 1; $i <= $qty; $i++) {
                                    if (isset($group['traits'])) {
                                        ...
                                        $roll = mt_rand(0, sizeof($grouptraits) - 1);

                                    } else {
                                        throw new \Exception('Cannot roll a group with no traits to roll.');
                                    }
                                    //get result
                                    $result = $grouptraits[$roll];
                                    //prep trait to be applied

                                    //check it exists
                                    $feature = Feature::find($result['trait_id']);
                                    if (!$feature) {
                        ...
                        if ($stack->item->tag('randomslot')->data['is_sellable'] == "true") {$characterData['is_sellable'] = true;} else {
                            $characterData['is_sellable'] = null;
                        }

                        if ($stack->item->tag('randomslot')->data['is_tradeable'] == "true") {$characterData['is_tradeable'] = true;} else {
                            $characterData['is_tradeable'] = null;
                        }

                        if ($stack->item->tag('randomslot')->data['is_giftable'] == "true") {$characterData['is_giftable'] = true;} else {
                            $characterData['is_giftable'] = null;
                        }

                        if ($stack->item->tag('randomslot')->data['is_visible'] == "true") {$characterData['is_visible'] = true;} else {
