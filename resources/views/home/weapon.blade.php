                ...
                <div class="row mb-3">
                    @foreach($chunk as $weaponId=>$stack)
                        <div class="col-sm-3 col-6 text-center inventory-item" data-id="{{ $stack->pivot->id }}" data-name="{{ $user->name }}'s {{ $stack->name }}">
                            ...
                            <div>
                                <a href="#" class="inventory-weapon inventory-weapon-name">{{ $stack->name }}</a> @if($stack->character_id) <i class="fas fa-lock mr-2"  data-toggle="tooltip" title="Attached to a character"></i> @endif
                            </div>
                        </div>
                    @endforeach
                </div>
