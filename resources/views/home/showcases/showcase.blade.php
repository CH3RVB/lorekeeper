...
@section('home.showcases-content')
    ...
    <h1>
        {{ $showcase->name }} <a href="{{ url('reports/new?url=') . $showcase->url }}"><i
                class="fas fa-exclamation-triangle fa-xs" data-toggle="tooltip"
                title="Click here to report this {{ __('showcase.showcase') }}."
                style="opacity: 50%; font-size:0.5em;"></i></a>
    </h1>
    <div class="mb-3">
    ...
    <div class="text-center">
        @if ($showcase->showcaseImageUrl)
            <img src="{{ $showcase->showcaseImageUrl }}" style="max-width:50%" alt="{{ $showcase->name }}" />
        @endif
        <p>{!! $showcase->parsed_description !!}</p>
    </div>

    @if (count($items))
        <h3> Items <a class="small inventory-collapse-toggle collapse-toggle collapsed" href="#itemstockcollapsible"
                data-toggle="collapse">Collapse View</a></h3>
                        ...
                        @foreach ($items as $categoryId => $categoryItems)
                            <li class="nav-item">
                                <a class="nav-link {{ $loop->first ? 'active' : '' }}"
                                    id="categoryTab-{{ isset($categories[$categoryId]) ? $categoryId : 'misc' }}"
                                    data-toggle="tab"
                                    href="#category-{{ isset($categories[$categoryId]) ? $categoryId : 'misc' }}"
                                    role="tab">
                ...
                <div class="card-body tab-content">
                    @foreach ($items as $categoryId => $categoryItems)
                        <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}"
                            id="category-{{ isset($categories[$categoryId]) ? $categoryId : 'misc' }}">
                                ...
                                <div class="row mb-3">
                                    @foreach ($chunk as $item)
                                        <div class="col-sm-3 col-6 text-center inventory-item"
                                            ...
                                            <div>
                                                <a href="#"
                                                    class="inventory-stack inventory-stack-name"><strong>{{ $item->name }}</strong></a>
    ...
    @if (count($pets))
        <h3> Pets <a class="small inventory-collapse-toggle collapse-toggle collapsed" href="#petstockcollapsible"
                data-toggle="collapse">Collapse View</a></h3>
        <div class="card mb-3 inventory-category collapse show" id="petstockcollapsible">
            <div class="card-body inventory-body">
                        ...
                        @foreach ($pets as $categoryId => $categoryItems)
                            <li class="nav-item">
                                <a class="nav-link {{ $loop->first ? 'active' : '' }}"
                                    id="categoryTab-{{ isset($categories[$categoryId]) ? $categoryId : 'misc' }}"
                                    data-toggle="tab"
                                    href="#petCategorythingy-{{ isset($petCategories[$categoryId]) ? $categoryId : 'misc' }}"
                                    role="tab">
                ...
                <div class="card-body tab-content">
                    @foreach ($pets as $categoryId => $categoryItems)
                        <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}"
                            id="petCategorythingy-{{ isset($petCategories[$categoryId]) ? $categoryId : 'misc' }}">
                            @foreach ($categoryItems->chunk(4) as $chunk)
                                <div class="row mb-3">
                                    @foreach ($chunk as $item)
                                        <div class="col-sm-3 col-6 text-center inventory-item"
                                            data-id="{{ $item->pivot->id }}">
                                            <div class="mb-1">
                                                <a href="#" class="inventory-stack"><img
